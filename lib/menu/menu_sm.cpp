...
MenuSM::MenuSM(Display *display, Event *event)
	: StateMachine(STATE_CAST(&MenuSM::stateStartup)), display_(display), game_(nullptr), load_following_state_(nullptr), load_following_entry_mode_(ForwardEntry)
{
...
void MenuSM::stateResetMenu(Event *event)
{
	...
	display_->clear();
	display_->text1_.setText((language_ == EN ? "reset scores" : "Highscores zur"
																 "\x1c"
																 "cksetzen"),
							 false);
