...
Tetris::Tetris(Display *display) : Game(display, TETRIS), field_(display->rows()), tetromino_(), step_timer_(nullptr), blink_timer_(nullptr), move_trigger_(nullptr), points_(0), blink_cycle_(DEFAULT), blink_start_row_(0), blink_end_row_(0)
{
	Tetris::setSpeed(2);
...
{
	step_timer_->setInterval(readSpeed((event->buttonState(BUTTON_DOWN) ? DownInterval : StepInterval)));
	step_timer_->start();
	move_trigger_->start();
...
uint16_t Tetris::readSpeed(const Tetris::SpeedFlag flag) const
{
