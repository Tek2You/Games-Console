...
uint16_t SpaceInvaders::readSpeed(const SpaceInvaders::SpeedFlag flag) const {
...
bool SpaceInvaders::onTriggered(Event* event) {
	...
	if (step_timer_->triggered()) {
		...
		insertRow();
		// check for shots
		for (uint16_t i = 0; i < shots_.size(); i++) {
...
Shot::Shot() : row_(0), col_(0) {}

Shot::Shot(byte col) : col_(col) {
	row_ = 2;
}
